const NGROK_PUBLIC_URL = '7f26-176-88-67-127.ngrok-free.app'; // Burayı her zaman güncel tut!

// YENİ: Bildirim fonksiyonu
function showNotification(title, options, urlToOpen = null) { // windowName parametresi eklendi
        ...
        if (Notification.permission === "granted") {
            ...
            if (urlToOpen) {
                notification.onclick = function() {
                    // Eğer windowName verilmişse, o isimdeki pencereye odaklan veya yeni aç
                    // Aksi takdirde varsayılan olarak yeni sekme (_blank) aç
                    window.open(urlToOpen).focus(); // .focus() eklendi
                    notification.close();
                };
            }
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // --- YENİ: Sayfa yüklendiğinde bildirim izni isteyelim ---
    ...
    notificationSocket.onmessage = function(e) {
        ...
        if (data.type === 'notification') {
            ...
            const is_dm_room = data.is_dm_room;
            
            if (document.hidden) {
                let notificationTitle = '';
                let notificationIcon = ''; // Bu satır eksikti, ekledim
                if (is_dm_room) {
                    notificationTitle = `${sender}`; // Gönderenin adı başlıkta
                    notificationIcon = 'static/person.svg';
                } else {
                    notificationTitle = `${slug}`; // Kanal adı başlıkta (isteğe bağlı olarak göndereni de ekleyebilirsiniz)
                    notificationIcon = 'static/people.svg';
                }
                const notificationOptions = {
                    body: is_dm_room ? lastMessage : `${sender}: ${lastMessage}`, // Mesaj içeriği gövdede
                    icon: notificationIcon,
                    tag: slug
                };
                const urlToOpen = `/chat/${slug}/`;
                
                
                // *** DÜZELTME BURADA: windowName parametresi eklendi ***
                showNotification(notificationTitle, notificationOptions, urlToOpen); 
            }
